...
- Added post-transcription transcript display in the popup using full normalized text and a compact Command-V paste hint.
- Changed popup dismiss timing to scale with transcript character count (clamped for readability), with a short fade-out.
- Kept quick dictation states focused on `Listening` and `Transcribing locally` while showing the newest completed transcript when sessions happen back-to-back.
- Reworked dev test builds to package only the current source tree, use isolated dev preferences, and create timestamped `VachaVox Dev` app bundles with a latest-build copy.
- Simplified Hotkeys settings to one Keyboard Settings button while keeping the recommended Fn setup guidance.

### Docs

- Updated `AGENTS.md` to use an unreleased-first development flow and explicit release-trigger versioning/packaging rules.
- Added `Docs/development/module-map.md` for module boundaries and parallel-safe development guidance.
- Added `Docs/patch-workflow-reference.md` as optional legacy guidance while patch workflow remains inactive.
...
- Fixed Fn trigger release handling so the default Fn shortcut works consistently in hold-to-dictate and toggle mode.
- Removed unused Carbon hotkey registration code now that normal key combinations use KeyboardShortcuts.

### Improvements

- Added Hotkeys settings guidance for the tested macOS setup: Dictation set to Press Mic / F5 and Press Globe key set to Do Nothing.
- Added Hotkeys settings buttons for macOS Keyboard Shortcuts and Keyboard settings.
